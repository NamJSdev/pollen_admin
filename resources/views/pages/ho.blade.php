@extends('layouts.app')
@section('title', 'Danh sách các họ')
...
@section('content')
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                    <div>
                        <h4 class="mb-3">Danh Sách Các Họ</h4>
                    </div>
                    <div class="iq-search-bar device-search w-50 d-flex flex-wrap justify-content-end">
                        <form id="searchForm" action="{{ route('Ho') }}" method="GET" class="searchbox d-flex">
                            <div>
                                <a class="search-link" href="#"><i class="ri-search-line"></i></a>
                                <input type="text" id="searchInput"
                                    style="border-top-right-radius: 0%;border-bottom-right-radius: 0%;"
                                    class="text search-input" placeholder="Tìm kiếm theo tên họ..." name="search"
                                    value="" />
                            </div>
                        ...
                        <a href="" class="btn btn-primary add-list" data-toggle="modal" data-target="#createModal"><i
                                class="las la-plus"></i>Thêm Mới Họ</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-12">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center list-action">
                                            <a class="badge bg-success mr-2 edit" href="#" title="Edit"
                                                ...
                                                data-name="{{ $data->ten }}" data-desc="{{ $data->moTa }}" data-bo="{{$data->bo->id}}"><i
    ...
    <div id="createModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="createForm" action="{{ route('ThemHo') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h4 class="modal-title">Thêm Họ</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <label>Bộ</label>
                            <select name="bo" class="selectpicker form-control" data-style="py-0"
                                data-live-search="true">
                                <option value="">Chọn Bộ</option>
                                @foreach ($bo_datas as $data)
    ...
    <div id="editModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="editForm" action="{{ route('SuaHo') }}" method="POST">
                    @csrf
                    <div class="modal-header">
    ...
    <div id="deleteModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="deleteForm" action="{{ route('XoaHo') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h4 class="modal-title">Xóa Họ</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#editModal').on('show.bs.modal', function(event) {
                ...
                modal.find('select[name="bo"]').val(bo).change();
            });
