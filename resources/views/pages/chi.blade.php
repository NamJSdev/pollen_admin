@extends('layouts.app')
@section('title', 'Danh sách các chi')
...
@section('content')
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                    <div>
                        <h4 class="mb-3">Danh Sách Các Chi</h4>
                    </div>
                    <div class="iq-search-bar device-search w-50 d-flex flex-wrap justify-content-end">
                        <form id="searchForm" action="{{ route('Chi') }}" method="GET" class="searchbox d-flex">
                            <div>
                                <a class="search-link" href="#"><i class="ri-search-line"></i></a>
                                <input type="text" id="searchInput"
                                    style="border-top-right-radius: 0%;border-bottom-right-radius: 0%;"
                                    class="text search-input" placeholder="Tìm kiếm theo tên chi..." name="search"
                                    value="" />
                            </div>
                        ...
                        <a href="" class="btn btn-primary add-list" data-toggle="modal" data-target="#createModal"><i
                                class="las la-plus"></i>Thêm Mới Chi</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-12">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center list-action">
                                            <a class="badge bg-success mr-2 edit" href="#" title="Edit"
                                                ...
                                                data-name="{{ $data->ten }}" data-desc="{{ $data->moTa }}" data-ho="{{$data->ho->id}}"><i
    ...
    <div id="createModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="createForm" action="{{ route('ThemChi') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h4 class="modal-title">Thêm Chi</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <label>Họ</label>
                            <select name="ho" class="selectpicker form-control" data-style="py-0"
                                data-live-search="true">
                                <option value="">Chọn Họ</option>
                                @foreach ($ho_datas as $data)
    ...
    <div id="editModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="editForm" action="{{ route('SuaChi') }}" method="POST">
                    @csrf
                    <div class="modal-header">
    ...
    <div id="deleteModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="deleteForm" action="{{ route('XoaChi') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h4 class="modal-title">Xóa Chi</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">
                        <p>Bạn có chắc chắn muốn xóa chi này?</p>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#editModal').on('show.bs.modal', function(event) {
                ...
                modal.find('select[name="ho"]').val(ho).change();
            });
