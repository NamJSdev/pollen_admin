@extends('layouts.app')
@section('title', 'Danh sách cấu hình bề mặt')
...
@section('content')
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                    <div>
                        <h4 class="mb-3">Danh Sách Cấu Hình Bề Mặt</h4>
                    </div>
                    <div class="iq-search-bar device-search w-50 d-flex flex-wrap justify-content-end">
                        <form id="searchForm" action="{{route('BeMat')}}" method="GET"
                            class="searchbox d-flex">
                            <div>
                                <a class="search-link" href="#"><i class="ri-search-line"></i></a>
                                <input type="text" id="searchInput"
                                    style="border-top-right-radius: 0%;border-bottom-right-radius: 0%;"
                                    class="text search-input" placeholder="Tìm kiếm theo tên bề mặt..." name="search"
                                    value="" />
                            </div>
                        ...
                        <a href="" class="btn btn-primary add-list" data-toggle="modal" data-target="#createModal"><i
                                class="las la-plus"></i>Thêm Mới Bề Mặt</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-12">
    ...
    <div id="createModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="createForm" action="{{ route('ThemBeMat') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h4 class="modal-title">Thêm Bề Mặt</h4>
    ...
    <div id="editModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="editForm" action="{{ route('SuaBeMat') }}" method="POST">
                    @csrf
                    <div class="modal-header">
    ...
    <div id="deleteModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="deleteForm" action="{{ route('XoaBeMat') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h4 class="modal-title">Xóa Bộ</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">
                        <p>Bạn có chắc chắn muốn xóa bề mặt này?</p>
